...
async function processImages(folder: string, imageType: ImageType): Promise<UploadResult> {
...
async function main() {
  ...
  // Summary
  const totalSuccess = prodottiResult.success.length + trattamentiResult.success.length;
  const totalFailed = prodottiResult.failed.length + trattamentiResult.failed.length;
  ...
  if (totalFailed > 0) {
    ...
    const failedPaths = [...prodottiResult.failed, ...trattamentiResult.failed].map((f) => f.path);
    fs.writeFileSync(path.join(scriptsDir, 'failed-uploads.txt'), failedPaths.join('\n'));
