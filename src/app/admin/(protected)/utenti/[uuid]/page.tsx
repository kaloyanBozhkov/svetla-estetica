...
export default async function UserDetailPage({ params }: Props) {
  ...
  return (
    <div>
      <div className="mb-8">
        <Link href="/admin/utenti" className="text-sm text-gray-500 hover:text-gray-700">
          ← Torna agli utenti
        </Link>
        <h1 className="font-display text-3xl font-bold text-gray-900 mt-2">Dettagli Utente</h1>
      </div>

      <div className="grid gap-8 lg:grid-cols-3">
        {/* User Info */}
        <Card>
          <h2 className="font-display text-xl font-bold text-gray-900 mb-4">Informazioni</h2>
            ...
            {user.orders.length === 0 ? (
              ...
              <div className="space-y-4">
                {user.orders.map((order) => (
                  <div key={order.id} className="p-4 rounded-lg bg-gray-50 border border-gray-100">
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        <p className="font-medium text-gray-900">Ordine #{order.id}</p>
                ...
                {user.bookings.map((booking) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <p className="font-bold text-primary-600">{formatPrice(booking.price)}</p>
                        <span
                          className={`inline-block mt-1 text-xs px-2 py-1 rounded ${
