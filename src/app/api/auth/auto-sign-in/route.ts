...
export async function GET(request: Request) {
  ...
  if (!order || !order.user) {
    return NextResponse.redirect(`${env.NEXT_PUBLIC_BASE_URL}/ordini/${orderUuid}?success=true`);
  }

  // Verify the token (use order uuid + user id as a simple verification)
  const expectedToken = Buffer.from(`${orderUuid}:${order.user.id}`).toString('base64');
